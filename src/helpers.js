  ...
  if (addEvents)
    tile.addEventListener(
      ...
      (e) => {
        ...
        if (currentMember === false) return false;
        ...
        currentMember.location.layer = getRandomLayer()
          ? getRandomLayer()
          : null;
        ...
        members.placeMember(currentMember, scaleValue[1], 'click');
...
function getRandomLayer() {
  ...
  const randomLayerId =
    layers[Math.ceil(Math.random(layers.length) * layers.length) - 1];
  const randomLayer = document.querySelector(`#${randomLayerId[0]}`);

  if (randomLayer == null) return false;
...
function fillWaitingQueue(arr, num) {
  ...
  memberContainer.id = 'member-containter';

  memberContainer.style.gridTemplateColumns = `repeat(${game.board.members}, auto)`;

  document.body.appendChild(memberContainer);

  for (let i = 1; i <= num; i++) {
    const newMemberCell = document.createElement('div');
    newMemberCell.id = `w-cell-${i}`;
    newMemberCell.classList.add('cell');
    memberContainer.appendChild(newMemberCell);
...
function moveToOnStageQueue(e) {
  ...
  if (game.onStageQueue.length === game.board.members) return false;

  let memberToMove = game.waitingQueue.pop();

  memberToMove.anchor.dataset.queueId =
    game.onStageQueue.push(memberToMove) - 1;
